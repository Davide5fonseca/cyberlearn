...
export default function ProfessorDashboard({ theme, user }) {
  ...
  const carregarDados = async () => {
    try {
      ...
      // Validar res.ok e o formato: uma resposta de erro ({erro}) deixaria
      // `acessos` sem ser array e rebentava o render com TypeError.
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      if (Array.isArray(data)) setAcessos(data);
    } catch (error) {
  ...
  const handleExportPDF = async () => {
    try {
      await exportarPdf({
        variante: 'relatorio',
        titulo: 'Relatório Oficial de Atividade da Turma',
        subtitulo: `Data de Emissão: ${new Date().toLocaleDateString('pt-PT')} | Professor: ${user?.nome || 'N/A'}`,
        colunas: ['ID', 'Nome do Aluno', 'Endereço de E-mail', 'Data e Hora', 'Estado'],
        linhas: acessos.map(a => [`#${a.id}`, a.nome, a.email, a.data, 'Sucesso']),
        assinatura: 'O Professor Responsável,',
        nomeFicheiro: 'CyberLearn_Relatorio_Acessos_Turma.pdf',
      });
    } catch (error) {
  ...
  const cabecalhosTabela = [t('profDash.aluno'), t('admin.email'), t('admin.timestamp'), t('admin.status')];
      ...
      <div style={styles.layout}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', flexShrink: 0 }}>
          ...
          >
            ...
            {t('profDash.exportPdf')}
          </button>
        </div>

        <div style={styles.cardContainer}>
          <div style={{ padding: '24px', borderBottom: `1px solid ${theme.inputBorder}`, flexShrink: 0 }}>
            <h2 style={{ color: theme.textMain, margin: '0 0 8px 0', fontSize: '18px', fontWeight: '700' }}>{t('profDash.historicoTurma')}</h2>
            <p style={{ color: theme.textSub, fontSize: '14px', margin: 0 }}>
              {t('profDash.fullHistoryDesc')}
            </p>
          </div>
  ...
  return (
    <div style={styles.layout}>
      ...
      <div style={styles.metricGrid}>
        <MetricCard
          theme={theme}
          etiqueta={t('profDash.alunosAtivos')}
          valor={alunosAtivos}
      ...
      <div style={styles.cardContainer}>
        ...
        <div style={{ padding: '24px', borderBottom: `1px solid ${theme.inputBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', flexShrink: 0 }}>
          <div>
            <h2 style={{ color: theme.textMain, margin: '0 0 8px 0', fontSize: '18px', fontWeight: '700' }}>{t('profDash.ultimosAcessos')}</h2>
            <p style={{ color: theme.textSub, fontSize: '14px', margin: 0 }}>
              {t('profDash.monitorDesc').replace('{n}', acessosExibidos.length)}
            </p>
          </div>
        ...
        <TabelaAcessos
          ...
          textoVazio={t('profDash.noData')}
          loading={loading}
          textoCarregar={t('profDash.aCarregar')}
        />
      </div>
    </div>
  );
